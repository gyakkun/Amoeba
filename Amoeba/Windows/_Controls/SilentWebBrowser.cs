...
{
    // http://stackoverflow.com/questions/4412915/how-to-deactivate-right-click-on-wpf-webbrowser-control
    public static class SilentWebBrowser
    {
        public static WebBrowser Create()
        {
            var webBrowser = new WebBrowser();
            var handler = new WebBrowserHostUIHandler(webBrowser);
            webBrowser.PreviewKeyDown += webBrowser_PreviewKeyDown;
            webBrowser.PreviewKeyUp += webBrowser_PreviewKeyUp;
            webBrowser.PreviewDragEnter += webBrowser_PreviewDragEnter;
            webBrowser.PreviewDragOver += webBrowser_PreviewDragOver;
            webBrowser.PreviewDrop += webBrowser_PreviewDrop;
        ...
        static void webBrowser_PreviewKeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            e.Handled = true;
        }

        static void webBrowser_PreviewKeyUp(object sender, System.Windows.Input.KeyEventArgs e)
        {
            e.Handled = true;
        }

        static void webBrowser_PreviewDragEnter(object sender, DragEventArgs e)
        {
            e.Handled = true;
        }

        static void webBrowser_PreviewDragOver(object sender, DragEventArgs e)
        {
            e.Handled = true;
        }

        static void webBrowser_PreviewDrop(object sender, DragEventArgs e)
        {
            e.Handled = true;
        }

        class WebBrowserHostUIHandler : Native.IDocHostUIHandler
        {
            ...
            public WebBrowserHostUIHandler(WebBrowser browser)
            {
                ...
                browser.LoadCompleted += this.OnLoadCompleted;
